...
<ol>
<li>
<p>动态线程池配置文件，建议单独开一个文件放到 apollo 中，如：user-center-dtp-dev.yml</p>
</li>
<li>
...
<h3 id="maven依赖" tabindex="-1"><a class="header-anchor" href="#maven依赖" aria-hidden="true">#</a> maven依赖</h3>
    ...
    <span class="token key atrule">tp</span><span class="token punctuation">:</span>
      ...
      <span class="token key atrule">executors</span><span class="token punctuation">:</span>                                   <span class="token comment"># 动态线程池配置，都有默认值，采用默认值的可以不配置该项，减少配置量</span>
        <span class="token punctuation">-</span> <span class="token key atrule">threadPoolName</span><span class="token punctuation">:</span> dtpExecutor1
          ...
          <span class="token key atrule">platformIds</span><span class="token punctuation">:</span> <span class="token punctuation">[</span><span class="token number">1</span><span class="token punctuation">,</span><span class="token number">2</span><span class="token punctuation">]</span>                             <span class="token comment"># 报警平台id，不配置默认拿上层platforms配置的所有平台</span>
...
<span class="token key attr-name">spring.dynamic.tp.apollo.namespace</span><span class="token punctuation">=</span><span class="token value attr-value">user-center-dtp-dev.properties</span>
...
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">apollo</span><span class="token punctuation">:</span>
  <span class="token key atrule">bootstrap</span><span class="token punctuation">:</span>
    <span class="token key atrule">enabled</span><span class="token punctuation">:</span> <span class="token boolean important">true</span>
    <span class="token key atrule">namespaces</span><span class="token punctuation">:</span> user<span class="token punctuation">-</span>center<span class="token punctuation">-</span>dev.yml<span class="token punctuation">,</span>user<span class="token punctuation">-</span>center<span class="token punctuation">-</span>dtp<span class="token punctuation">-</span>dev.yml  <span class="token comment"># 注意：这里要加上动态线程池配置文件，服务启动会根据此配置拉取配置	</span>
  <span class="token key atrule">meta</span><span class="token punctuation">:</span> http<span class="token punctuation">:</span>//localhost<span class="token punctuation">:</span><span class="token number">8080</span>
  <span class="token key atrule">config-service</span><span class="token punctuation">:</span> http<span class="token punctuation">:</span>//localhost<span class="token punctuation">:</span><span class="token number">8080</span>
<span class="token key atrule">app</span><span class="token punctuation">:</span>
  <span class="token key atrule">id</span><span class="token punctuation">:</span> user<span class="token punctuation">-</span>center
...
<p>如果 Apollo 使用 docker 方式安装，需要更改配置，参考这篇文章：https://blog.csdn.net/qq_35530042/article/details/113985623</p>
<p>docker 方式安装请参考这篇文章：https://www.apolloconfig.com/#/zh/deployment/quick-start-docker</p>
</div>
<h3 id="接入-apollo-配置流程" tabindex="-1"><a class="header-anchor" href="#接入-apollo-配置流程" aria-hidden="true">#</a> 接入 Apollo 配置流程</h3>
<ol>
<li>本地或 docker 安装 apollo 之后访问 <code v-pre>localhost:8070</code></li>
</ol>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231205191955993.png" alt="image-20231205191955993" tabindex="0" loading="lazy"><figcaption>image-20231205191955993</figcaption></figure>
<ol start="2">
<li>创建项目，appid 即为配置文件里的 app.id</li>
</ol>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231205192141255.png" alt="image-20231205192141255" tabindex="0" loading="lazy"><figcaption>image-20231205192141255</figcaption></figure>
<ol start="3">
<li>新建 namespace，除非您想使用默认的 application 作为 namespace name，此时您的配置文件里应该将 namespace 设置为 namespace: application 或者不配置</li>
</ol>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231206102730485.png" alt="image-20231206102730485" tabindex="0" loading="lazy"><figcaption>image-20231206102730485</figcaption></figure>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231205192713419.png" alt="image-20231205192713419" tabindex="0" loading="lazy"><figcaption>image-20231205192713419</figcaption></figure>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231205192944618.png" alt="image-20231205192944618" tabindex="0" loading="lazy"><figcaption>image-20231205192944618</figcaption></figure>
<ol start="4">
<li>创建 namespace 后将 dynamic-tp 动态线程池配置文件放置在配置中心 apollo 里</li>
</ol>
<figure><img src="@source/guide/configcenter/apollo.assets/image-20231205193214034.png" alt="image-20231205193214034" tabindex="0" loading="lazy"><figcaption>image-20231205193214034</figcaption></figure>
</div></template>
