...
<ol>
<li>
<p>动态线程池配置文件，建议单独开一个文件放到 servicecomb 中，如：user-center-dtp-dev.yml</p>
</li>
<li>
...
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  ...
  <span class="token key atrule">cloud</span><span class="token punctuation">:</span>
    <span class="token key atrule">servicecomb</span><span class="token punctuation">:</span>
      <span class="token key atrule">config</span><span class="token punctuation">:</span>
        <span class="token comment"># 配置中心地址，本示例使用ServiceStage环境变量。建议保留这种配置方式，</span>
        <span class="token comment"># 部署的时候，不用手工修改地址。</span>
        <span class="token key atrule">serverAddr</span><span class="token punctuation">:</span> $<span class="token punctuation">{</span>PAAS_CSE_CC_ENDPOINT<span class="token punctuation">:</span>http<span class="token punctuation">:</span>//127.0.0.1<span class="token punctuation">:</span><span class="token number">30110</span><span class="token punctuation">}</span>
        <span class="token key atrule">serverType</span><span class="token punctuation">:</span> kie
        <span class="token comment"># 自定义配置</span>
        <span class="token key atrule">kie</span><span class="token punctuation">:</span>
          <span class="token key atrule">customLabel</span><span class="token punctuation">:</span> public
          <span class="token key atrule">customLabelValue</span><span class="token punctuation">:</span> default
        <span class="token comment"># 自定义配置，使用文本的key/value配置项作为yaml格式配置</span>
        <span class="token key atrule">fileSource</span><span class="token punctuation">:</span> application.yaml
