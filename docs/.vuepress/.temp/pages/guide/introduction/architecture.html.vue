...
<p>4.example 模块：提供一个简单使用示例，方便使用者参照</p>
<p>5.extension 模块：放一些扩展功能实现，比如基于 redis 的流控扩展、邮件发送扩展、skywalking 上下文传递扩展等</p>
<p>6.logging 模块：用于配置框架内部日志的输出，目前主要用于输出线程池监控指标数据到指定文件</p>
...
<p>1.服务启动时从配置中心拉取配置，生成线程池实例注册到内部线程池注册中心以及 Spring 容器中</p>
<p>2.接受配置监听模块的刷新事件，实现线程池参数的刷新</p>
<p>3.代码中通过依赖注入（推荐）或者 DtpRegistry.getDtpExecutor() 方法根据线程池名称来获取线程池实例</p>
