export const data = {
  ...
  "frontmatter": {
    ...
    "sticky": true,
    "star": true,
    "summary": "线程池定义 建议直接配置在配置中心，但是如果想后期再添加到配置中心，可以先用 @Bean 编码式声明（方便 Spring 依赖注入） 代码引用 从 DtpRegistry 中根据线程池名称获取，或者通过依赖注入方式（推荐，更优雅） 依赖注入方式使用，优先推荐依赖注入方式，不能使用依赖注入的场景可以使用方式2; 从 DtpRegistry 注册器获取（接口场景",
    "head": [
      [
        "meta",
        {
