...
${u.blue("Please consider disable `cacheHTML` and `cachePic`, or set `maxSize` and `maxPicSize` option.\n")}`)})},d=(e,a)=>C(a)?a.replace(/^\//,e):a,M=(e,a)=>(a.icons&&(a.icons=a.icons.map(n=>({...n,src:d(e,n.src)}))),a.shortcuts&&(a.shortcuts=a.shortcuts.map(n=>{var i;return{...n,icons:((i=n.icons)==null?void 0:i.map(l=>({...l,src:d(e,l.src)})))||[],url:d(e,n.url)}})),a.screenshots&&(a.screenshots=a.screenshots.map(n=>({...n,src:d(e,n.src)}))),a),F=(e,a)=>{a.favicon&&(a.favicon=d(e,a.favicon)),a.apple&&(a.apple.icon&&(a.apple.icon=d(e,a.apple.icon)),a.apple.maskIcon&&(a.apple.maskIcon=d(e,a.apple.maskIcon))),a.msTile&&a.msTile.image&&(a.msTile.image=d(e,a.msTile.image)),a.manifest&&(a.manifest=M(e,a.manifest))},K=(e,a="/",n=[])=>{var i;const l=[],p=[];n.forEach(o=>{o[0]==="meta"?l.push(o[1].name):o[0]==="link"&&p.push(o[1].rel)});let t="";const s=(o,c,g={})=>{p.includes(o)||n.push(["link",{rel:o,href:c,...g}])},r=(o,c,g={})=>{l.includes(o)||n.push(["meta",{name:o,content:c,...g}])};if(e.favicon&&s("icon",e.favicon),(i=e.manifest)!=null&&i.icons){const{icons:o}=e.manifest;o.length>0&&(t=o[0].src,e.manifest.icons.map(c=>{c.type?s("icon",c.src,{type:c.type,sizes:c.sizes}):s("icon",c.src,{sizes:c.sizes})}))}return s("manifest",`${a}manifest.webmanifest`,{crossorigin:"use-credentials"}),r("theme-color",e.themeColor||"#46bd87"),f(e.apple)&&(e.apple.icon||t)?(s("apple-touch-icon",e.apple.icon||t),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style",e.apple.statusBarColor||"black"),e.apple.maskIcon&&s("mask-icon",e.apple.maskIcon,{color:e.themeColor||"#46bd87"})):e.apple!==!1&&t&&(s("apple-touch-icon",t),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style","black")),f(e.msTile)&&(e.msTile.image||t)?(r("msapplication-TileImage",e.msTile.image||t),r("msapplication-TileColor",e.msTile.color||e.themeColor||"#46bd87")):e.msTile!==!1&&t&&(r("msapplication-TileImage",t),r("msapplication-TileColor",e.themeColor||"#46bd87")),r("viewport","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover"),n},_=(e,a)=>{let n="",i="";return a.showInstall&&(n+=`import PWAInstall from "${k}components/PWAInstall.js";
`,i+=`PWAInstall,
`),a.update==="hint"?(n+=`import SWHintPopup from "${a.hintComponent||`${k}components/SWHintPopup.js`}";
`,i+=`SWHintPopup,
`):a.update!=="disable"&&a.update!=="force"&&(n+=`import SWUpdatePopup from "${a.updateComponent||`${k}components/SWUpdatePopup.js`}";
`,i+=`SWUpdatePopup,
...
`)},U=(e={},a=!0)=>n=>{a&&W(e),P(n,v,"2.0.0-beta.61"),n.env.isDebug&&m.info("Options:",e);const{base:i,shouldPrefetch:l=!0}=n.options;e.appendBase&&F(i,e),l===!0&&m.warn('The plugin will register service worker to handle assets, so we recommend you to set "shouldPrefetch: false" in VuePress config file.');const p=B(n,e);return n.options.head=K(e,i,n.options.head),I(n,{id:"hope"}),{name:v,define:()=>({PWA_LOCALES:A({app:n,name:v,default:w,config:e.locales}),SW_FORCE_UPDATE:e.update==="force",SW_PATH:e.swPath||"service-worker.js"}),extendsBundlerOptions:(t,s)=>{T(t,s,["mitt","register-service-worker"]),O(t,s,{path:"/manifest.webmanifest",response:async(r,o)=>(o.setHeader("Content-Type","application/manifest+json"),JSON.stringify(await p)),errMsg:"Unexpected manifest generate error"})},onGenerated:async t=>{await L(t,p),await H(t,e)},clientConfigFile:t=>_(t,e)}};export{w as pwaLocales,U as pwaPlugin};
