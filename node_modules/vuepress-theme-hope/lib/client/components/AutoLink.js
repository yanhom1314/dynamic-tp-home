...
export default defineComponent({
    ...
    setup(props, { attrs, emit, slots }) {
        ...
        const linkTarget = computed(() => hasNonHttpProtocol.value
            ? undefined
            : config.value.target || (hasHttpProtocol.value ? "_blank" : undefined));
        // if the `target` attr is "_blank"
        const isBlankTarget = computed(() => linkTarget.value === "_blank");
        // render `<VPLink>` or not
        const renderVPLink = computed(() => !hasHttpProtocol.value &&
            !hasNonHttpProtocol.value &&
            !isBlankTarget.value);
        // resolve the `rel` attr
        const anchorRel = computed(() => hasNonHttpProtocol.value
            ? undefined
            : config.value.rel ||
        ...
        const isActive = computed(() => renderVPLink.value
